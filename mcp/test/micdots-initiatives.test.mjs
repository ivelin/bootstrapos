...
describe("PR2.1 micdots Heavy-locked initiatives map", () => {
  ...
  it("recon put_journey writes initiatives[] and does not Advance", async () => {
    ...
    assert.match(seen.spoken, /Bottleneck #1: paid construction work puts the first check in the company/);
    assert.match(seen.ideas[0].snapshot, /Bottleneck #1: paid construction work puts the first check in the company/);
    ...
    const spokenLead = seen.spoken.split("Also moving")[0];
    assert.match(spokenLead, /paid construction work puts the first check in the company/);
    assert.match(spokenLead, /Granite Design intro/);
    assert.doesNotMatch(spokenLead, /FAST|SOPA|angels|legal/i);
  });
});
