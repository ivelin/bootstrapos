...
| `create_idea` | founder + founder-authorized | New 0-1 board under a held company. Empty clocks (Write the bet / hold; stored 1 / 1). Why must be a named-group kill line (pay or use). A person, hire, or instrument is not an idea board. Founder yes in chat. Does not invent stage. Ask / Do / Write back is a quality bar, not a card. |
| `put_journey` | founder + founder-authorized | Overwrite journey/gate/jsonb including `constraint_this_week` and scoreboard (hypothesis, open questions) on an **existing** idea. Missing slug → `idea not found; call create_idea first`. Phase write needs a product why (named-group kill line) and founder yes in chat. Existing relationship/instrument slugs reject phase writes; snapshot says relationship shelf / not a journey. Constraint-only recon is still allowed. `loopStage` mutations and spoken Ask/Do/Write back writes are rejected. |
