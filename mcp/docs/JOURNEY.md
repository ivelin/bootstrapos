...
| `grant_super_admin` / `revoke_super_admin` | live `super_admin` | Grant or revoke another login. No self-grant (403). Revoke is immediate. Revoking the last live super_admin is 409 `last_super_admin` (no audit row). No self-serve signup. The first super_admin is not this tool: a first-time hosted install names a real contact, and the operator seeds that row by hand after the roles migration, because this RPC requires an existing super_admin caller. [`HOSTED_IDENTITY.md`](HOSTED_IDENTITY.md#first-super-admin-hosted-install). Audit the grant and the revoke. |
...
Listed on the production pin when the journey store is attached (Cos SQL applied). The self-host stdio kit does not register them (do not lift local traces). Membership is `bootstrap_company_labels` — same companies as whoami. `whoami.role` is `member`, `super_admin`, or `unset`. Hosted callers do not call `bootstrap_init_company`; a super admin calls `create_company`, then `create_idea`. The operator applies `20260928_bootstrap_os_roles.sql` — not from a PR agent. That file does not seed a super_admin. A first-time hosted install names a real super_admin email contact; the operator inserts that row by hand, outside git, after the migration, because `grant_super_admin` cannot run when no super_admin exists yet. Template and test fixtures use `founder@example.test` as an illustrative `member` only, not a super_admin. Real contact addresses are never committed. Procedure: [`HOSTED_IDENTITY.md`](HOSTED_IDENTITY.md#first-super-admin-hosted-install).
