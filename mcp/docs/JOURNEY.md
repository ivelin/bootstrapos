...
| Tool | Who | Notes |
|------|-----|--------|
| `get_journey` | founder / advisor on the allowlist | Company query → every idea. Company/idea → one idea. Speak the card: company header → bottleneck #1 → customer checks with nested engagements → other initiatives footer. Surfaces `card` from `initiatives[]` (dual-read old supporting / engagements / constraintThisWeek), ACL `owners`, stored `portfolioScore` on live ideas, and `portfolio` ranked by impact+evidence+leverage when ≥2 live ideas. Never invents missing scores. `progress[]` is not card body. |
