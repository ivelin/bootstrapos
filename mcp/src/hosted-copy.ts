...
3. Answer only from the payload. Include visualFlow mermaid so the client can render the journey in whatever style the user prefers. Speak the card: company header (stage, gate, WIP) → bottleneck #1 → customer checks with nested engagements → other initiatives footer. Dual-read old supporting / engagements / constraintThisWeek when initiatives[] is empty. Also clocks, snapshot, lastTransitions, comments, audit, openQuestions, owners, and portfolio (Impact/Evidence/Leverage on live ideas — never invent missing scores). NDA is not Try. progress[] is not card body. Spoken Ask/Do/Write back are a quality bar, not a card. Spoken board talk leads with descriptive labels; numbers in parentheses only if useful.
...
export const TOOL_GET_JOURNEY =
  "Where are we — the company board and ideas under it (separate boards). Use when the user says where are we, show the company board, show company X ideas, show my idea board, or similar. Speak the card: company header → bottleneck #1 → customer checks with nested engagements → other initiatives footer. Returns card, clocks, snapshot, visualFlow mermaid, decision log, bottleneck, open questions, owners, and portfolio scores. Dual-read old supporting / engagements / constraintThisWeek. Omit idea for every idea under the company. Uses the active company if already chosen. Do not invent a stage, log rows, or missing scores. progress[] is not card body. Do not use GitHub as the board. Spoken or rendered summary should lead with descriptive labels; numbers in parentheses."
