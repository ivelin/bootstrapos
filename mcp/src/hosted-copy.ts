...
3. Answer only from the payload. Include visualFlow mermaid so the client can render the journey in whatever style the user prefers. Speak the board in order: primary → supporting → engagements. Also clocks, snapshot, lastTransitions, comments, audit, constraintThisWeek, openQuestions, owners, supporting, engagements, and portfolio (Impact/Evidence/Leverage on live ideas — never invent missing scores). Where-are-we is journey + gate + constraint + missing artifacts, then supporting (no rungs), then engagements (NDA is not Try). Spoken Ask/Do/Write back are a quality bar, not a card. Spoken board talk leads with descriptive labels; numbers in parentheses only if useful.
...
create_idea starts a new 0-1 primary board under a company (empty clocks, hold). Several ideas are allowed; each is its own customer bet. A person, hire, FAST/SAFE, investor, contractor, or partner-as-the-bet is supporting — write supporting[] on the same snapshot (no rungs). Exile to a spreadsheet is rejected. put_journey writes an existing idea (bottleneck or Advance/Iterate/Hold/Kill, founder yes in this chat for journey/gate). Primary phase needs a product bet-class why (named-group kill line, pay or use). Recon may patch supporting, constraint, and progress; recon may not Advance primary. loopStage mutations and spoken Ask/Do/Write back writes are rejected. A missing idea is not a write — call create_idea first. post_comment never moves clocks. enable_board_watch turns on board updates for Bill after invite. If they ask for the decision log over time or to rebuild clocks at a past point, call list_provenance (same access as get_journey). Weekly portfolio labels are put_portfolio_score (impact, evidence, leverage 1–5, required short why). Scores never Advance or Kill. Single-idea boards skip ranking.
...
export const TOOL_GET_JOURNEY =
  "Where are we — the company board and ideas under it (separate boards). Use when the user says where are we, show the company board, show company X ideas, show my idea board, or similar. Speak primary → supporting → engagements. Returns clocks, snapshot, visualFlow mermaid, decision log, bottleneck, open questions, owners, supporting (no rungs), engagements (NDA is not Try), and portfolio scores. Omit idea for every idea under the company. Uses the active company if already chosen. Do not invent a stage, log rows, or missing scores. Do not use GitHub as the board. Spoken or rendered summary should lead with descriptive labels; numbers in parentheses."
...
export const TOOL_PUT_JOURNEY =
  "Update an existing 0-1 idea board (phase, gate, bottleneck this week) or patch supporting / engagements. New slugs: create_idea first. Primary phase needs a named-group kill line (pay or use) and an explicit founder yes, why, whatChanged, and whatWereNotDoing. Recon may patch supporting, constraint, and progress; recon may not Advance primary. Supporting has no rungs and cannot promote. loopStage mutations and spoken Ask/Do/Write back writes are rejected. Kill also requires a postmortem: lessonsLearned and actionableInsights."
