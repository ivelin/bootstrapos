...
create_idea starts a new 0-1 board under a company (empty clocks, hold). Several ideas are allowed; each is its own board. put_journey writes an existing idea (bottleneck or Advance/Iterate/Hold/Kill, founder yes in this chat for journey/gate). loopStage mutations and spoken Ask/Do/Write back writes are rejected. A missing idea is not a write — call create_idea first. post_comment never moves clocks. enable_board_watch turns on board updates for Bill after invite. If they ask for the decision log over time or to rebuild clocks at a past point, call list_provenance (same access as get_journey). Weekly portfolio labels are put_portfolio_score (impact, evidence, leverage 1–5, required short why). Scores never Advance or Kill. Single-idea boards skip ranking.
...
export const TOOL_CREATE_IDEA =
  "Start a new 0-1 idea board under a company this login can open. Empty clocks (Write the bet, hold). Ask / Do / Write back is a quality bar, not a card. Several ideas are allowed; each idea is its own board. Needs an explicit founder yes in this chat. Then put_journey writes that idea."

export const TOOL_PUT_JOURNEY =
  "Update an existing 0-1 idea board (phase, gate, bottleneck this week). New slugs: create_idea first. Phase or gate change needs an explicit founder yes, why, whatChanged, and whatWereNotDoing. loopStage mutations and spoken Ask/Do/Write back writes are rejected. Kill also requires a postmortem: lessonsLearned and actionableInsights."
